...
async function getAll() {
    const data = await knex("province");
    return data;
}

async function getById(id) {
    const data = await knex("province").where("id", id).first();
    return data;
}
...
async function create(info, transaction = null) {
    ...
    mutableTrimObj(data);
    await _knex("province").insert(data);
}
...
async function update(id, info, transaction = null) {
    ...
    const data = _.pick(info, fields);
    mutableTrimObj(data);
    await _knex("province").update(data).where("id", id);
}

async function remove(id, transaction = null) {
    const _knex = transaction || knex;
    await _knex("province").update({ status: 0 }).where("id", id);
}

async function hardDelete(id, transaction = null) {
    const _knex = transaction || knex;
    await _knex("province").where("id", id).del();
}
